...
void add_tray(std::string tray_name, std::vector<moveit_msgs::CollisionObject>& tray_components,moveit::planning_interface::PlanningSceneInterface& planning_scene){  
    ...
    if (!collision_objects.empty()) {
        std::cout << "Last object ID: " << collision_objects.back().id << std::endl;
    } else {
        std::cout << "collision_objects is EMPTY!" << std::endl;
    }
...
int main(int argc, char **argv){
 ...
 /*15*/ {-1,-1,-1,-1,-1,-1} // PAYLOAD TRAY
    ...
    tray_map= { {"tray_aoc", tray_components_aoc}, {"tray_dhc", tray_dhc}, {"tray_eps", tray_components_aoc},  
                {"tray_payload", tray_payload} }; 

    target_order_index=(start_index<8) ? start_index-1 : start_index-2;

    for (int i = start_index; i < value_tray_pose.size(); i++){

        tray_setup(i);

        if(contains(new_tray_indeces, i)){
            add_tray(tray_name,tray_map[tray_name],planning_scene);
            spawn_message(i);
            pick_sequence(kr120_arm_group, kr120_gripper_group, planning_scene, i);
        }

        if(tray_name=="tray_aoc" || tray_name=="tray_eps"){
            
            if(contains(operator_indeces, i)){
                operator_sequence(kr20_arm_group, kr120_arm_group, kr120_gripper_group, i);
            }

            if(!contains(new_tray_indeces, i) && !contains(operator_indeces, i)){
                recharge_sequence(kr20_arm_group, kr20_slide_group);
            }

            positioning_sequence(kr120_arm_group, kr120_gripper_group, i);

            if(contains(new_tray_indeces, i) || (i>start_index && contains(operator_indeces, i))){
                recharge_sequence(kr20_arm_group, kr20_slide_group);
            }

            internal_screwing_sequence(kr20_arm_group, kr20_slide_group, i);
        }
        place_sequence(kr20_arm_group, kr120_arm_group, kr120_gripper_group, planning_scene);


        if(contains(new_tray_indeces, i+1)){
            place_sequence(kr20_arm_group, kr120_arm_group, kr120_gripper_group, planning_scene);
           
            if(tray_name!="tray_aoc"){
                external_screwing_sequence(kr20_arm_group, kr20_slide_group, rl_plate_group, planning_scene); 
            }else{
                slide_sequence(rl_lifter_group,rl_slider_group,0.02,-1.0,0,0,planning_scene);
            }         
        }
    }

    slide_sequence(rl_lifter_group,rl_slider_group,0.02,-1.0,-1.0,0.02,planning_scene);
}